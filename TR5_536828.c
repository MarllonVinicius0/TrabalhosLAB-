...
static void heap_descida(void **elems, int P, COMP *comparador)
{
    ...
    int menor = pai_pos; 
    // O while ira comparar os pais com os filhos, escolhendo o menor deles e trocando.
    while (pai_pos < P - 1)
    {
...
HEAP* HEAP_create(int n, COMP* compara)
{
    ...
    // N  recebe n (parâmetro).
    heap->N = n;
    // P ( recebe zero para inicializar.
    heap->P = 0;
    // Aloca memória dos elentos.
    heap->elems = malloc(n * sizeof(void *));
    // Inicializando o vetor de 0 a n, vetor recebendo NULL.
    for(int i = 0; i < n; i++)
    {
        heap->elems[i] = NULL;
    }
