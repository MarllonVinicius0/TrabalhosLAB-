...
VETORORD* VETORD_create(int n, COMP* compara)
{
	...
	//dentro do for terá um ponteiro "p" que apontará para "elems" e realizará a soma com a multiplicação da variável "i" com o tamanho do ponteiro "void**"  sempré apontado para "NULL".
...
{
	//if para verificar se há espaço no vetor.
	if (vetor->P < vetor->N) 
	{
    	//if vai verificar a posição dos elementos.  
       	if (vetor->P == 0) 
            vetor->elems[0] = newelem;
        //else if vai verificar os elementos existentes no vetor implementado, compararando o newelem com os elementos do vetor.
        ...
        //o for sera oara "navegar" dentro do vetor e deslocar uma posição a frente.    
            for (int i = vetor->P-1; i > -1; --i) 
			{
        //o if  vai adicionar o novo elemento na posição.        
